...
@interface NJCreatNewFileManager ()
...
@implementation NJCreatNewFileManager
- (instancetype )init {
    if (self = [super init]) {
        self.classNamePrefix = @"NJ";
...
- (NSArray *)getRandomMethodArray {
    ...
    for (NSInteger i = 0; i<methodCout; i++) {
        ...
        NSDictionary *dict = @{
                               ...
                               @"value":[@"nj_" stringByAppendingString:[self nj_getRandonmString]]
                               };
...
- (NSString *)getRandomClassName {
    NSMutableString *className = [[NSMutableString alloc] initWithString:@""];
...
//     [className appendString:[self randomStringWithLength:arc4random()%3+1]];
    [className appendString:[self nj_getRandonmString]];
    return className;
}

- (NSString *)nj_getRandonmString{
    NSMutableString *randomString= [[NSMutableString alloc] initWithString:self.classNamePrefix];
